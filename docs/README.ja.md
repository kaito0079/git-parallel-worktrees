...
`.` は POSIX の書式で、bash / zsh では `source` と同じ。
...
`pwt sync` はメインリポジトリを指す既存のシンボリックリンクを削除してから貼り直す。worktree 内で完結する
リンクは残るため、worktree をリポジトリ内に置く構成でも、リポジトリが追跡しているシンボリックリンクは
消えない。`[copy]` の対象は、宛先に実ファイル/実ディレクトリがある場合はスキップされる。
...
- **OS**: macOS / Linux
- **依存**: git
- **シェル統合**: POSIX 互換シェルであればよい。`shell/pwt.sh` は bash 固有・zsh 固有の構文を使っておらず、bash / zsh / dash で検証している。
...
`internal/links` / `internal/cli` / `shell` のテストは、実際の git と実際のシェルを呼ぶ統合テストを含む。
